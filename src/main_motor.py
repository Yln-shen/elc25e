...
def main():
    ...
    cv2.createTrackbar('YAW_OFFSET', 'Control', 10, 50, lambda x: None)    # 范围0-50，初始10
    cv2.createTrackbar('PITCH_OFFSET', 'Control', 0, 50, lambda x: None)   # 范围0-50，初始0
    print("按 'q' 退出")
    ...
    try:
        while True:
            ...
            # 读取滑动条的值
            YAW_OFFSET = cv2.getTrackbarPos('YAW_OFFSET', 'Control')
            PITCH_OFFSET = cv2.getTrackbarPos('PITCH_OFFSET', 'Control')
            ...
            if use_pnp:
                ...
                _ = tracker.track(camera_center_offset)
            ...
            # ===== GPIO和电机控制 =====
            if camera_center_offset is not None or use_pnp:
            ...
            if camera_center_offset is not None or use_pnp:
                if use_pnp:
                    print(f"板子坐标: ({camera_center_offset[0] if camera_center_offset else 0:>7.1f}, {camera_center_offset[1] if camera_center_offset else 0:>7.1f})  [PNP]")
                    print(f"距离: {distance:.2f}m  ", end="")
                else:
                    print(f"板子坐标: ({camera_center_offset[0]:>7.1f}, {camera_center_offset[1]:>7.1f})  [Pixel]")
            ...
            if tracker.kf_position is not None:
                kf_x = int(detector.frame_center[0] + tracker.kf_position[0])
                kf_y = int(detector.frame_center[1] + tracker.kf_position[1])
                print(f"DEBUG: kf_pos=({tracker.kf_position[0]:.1f}, {tracker.kf_position[1]:.1f})")
                result = tracker.draw_kf(result, detector.frame_center)
                last_print_lines += 1
            else:
